...
extern long long int middleIOVolume = 0;

extern uint64 middlePDU = 0;

extern stxxl::timer reductionTimer = 0;

extern stxxl::timer inductionTimer = 0;
...
#ifdef COLLECT_STATISTICS
		reductionTimer.start();  // start counting reduction time
#endif
...
void DSAComputation<alphabet_type, offset_type, D>::run() {
	...
	if (is_unique == false) {
		...
		if (MAX_MEM >= m_s1->size() * (sizeof(uint32) + sizeof(uint32) + sizeof(uint32) + (double)1 / 8)) {
			...
			stxxl::stats *Stats = stxxl::stats::get_instance(); // collect reduction IO volume

			middleIOVolume = Stats->get_written_volume() + Stats->get_read_volume();

			stxxl::block_manager *bm = stxxl::block_manager::get_instance();
		
			middlePDU = bm->get_maximum_allocation(); 
#endif
			...
			reductionTimer.stop(); // stop counting reduction time

			inductionTimer.start(); // start counting induction time
#endif
		}
		else {
...
void DSAComputation<alphabet_type, offset_type, D>::mergeSortedSuffixGlobal() {
	...
	for (uint8 i = 0; i < m_blocks_info.size(); ++i) {
		...
		if (m_blocks_info[i].is_multi()) { // multi-block
			...
			is_l_star[i] = true; // single-blocks contain one L*-type suffix
		}
		else { // leftmost block contains at most one L*-type suffix
			...
			is_l_star[i] = (m_s_bwt_seqs[i]->size() == 0) ? false : true;
		}
	}
