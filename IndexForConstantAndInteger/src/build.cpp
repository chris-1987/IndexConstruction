...
int main(int argc, char** argv){
	...
	std::cerr << "reduction pdu\t total: " << middlePDU << "\t per char: " << (double)middlePDU / s_size << std::endl;

	uint64 reduction_io_volume = middleIOVolume - stats_begin.get_written_volume() - stats_begin.get_read_volume();

	std::cerr << "reduction io\t total: " << reduction_io_volume << "\t per char: " << (double)reduction_io_volume / s_size << std::endl;

	uint64 induction_io_volume = io_volume - middleIOVolume;

	std::cerr << "induction io\t total: " << induction_io_volume << "\t per char: " << (double)induction_io_volume / s_size << std::endl;

	std::cerr << "reduction time\t total: " << (reductionTimer.seconds()) << "\t per char: " << (double)(reductionTimer.seconds()) / s_size << std::endl;

	std::cerr << "induction time\t total: " << (inductionTimer.seconds()) << "\t per char: " << (double)(inductionTimer.seconds()) / s_size << std::endl;
